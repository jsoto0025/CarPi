...
namespace DataAccess.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserRepository(CarPiContext context)
        {
            _context = context;
        }
        ...
        public Task<IQueryable<User>> SearchUsersByTerm(string serachTerm)
        {
            ...
            {
                result = result.Where(x => x.Name.Contains(serachTerm));
            }
        ...
        public async Task Update(User model)
        {
            ...
            {
                originalData.Password = Cryptography.Sha1(model.Password);
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(name) || password == null)
            {
                return Task.FromResult((User)null);
            }

            password = Cryptography.Sha1(password);
